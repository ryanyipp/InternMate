...
// ✅ Use Vercel/Local env var for backend base URL (fallback to localhost for dev)
const BACKEND_BASE =
  (import.meta.env.VITE_API_URL || "http://localhost:3000").replace(/\/$/, "");

// Your backend routes are mounted at /api (app.use('/api/...'))
const API = axios.create({ baseURL: `${BACKEND_BASE}/api` });

// External API for fetching internships from RapidAPI (unchanged)
...
export const updateInternshipStatus = (userId, internshipId, status) =>
  API.patch(`/internships/${userId}/internship/${internshipId}/status`, {
    status,
  });
...
export const getExternalInternships = async () => {
  try {
    ...
    console.log(
      ...
      typeof jobsData,
      "Length:",
      Array.isArray(jobsData) ? jobsData.length : "Not an array"
    );
    ...
    const transformedJobs = jobsData.map((job, index) =>
      transformExternalJob(job, index)
    );
...
export const getRecommendedInternships = async (skills = [], location = "") => {
  try {
    ...
    if (skills.length > 0) {
      ...
      console.log(
        `🔍 After skills filter: ${internships.length} (from ${beforeSkillFilter})`
      );
    }
    ...
    if (location && location !== "All") {
      ...
      console.log(
        `📍 After location filter: ${internships.length} (from ${beforeLocationFilter})`
      );
    }
